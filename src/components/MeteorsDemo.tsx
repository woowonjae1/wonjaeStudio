...
function MeteorsCardFallback(
  props: Omit<MeteorsCardProps, "children"> & { children: React.ReactNode }
) {
  return (
    <div
      className={`relative h-full w-full overflow-hidden rounded-2xl bg-slate-900 p-4 ${props.className}`}
    >
...
export function MeteorsCard({
  ...
  try {
    return (
      <div
        className={`relative h-full w-full overflow-hidden rounded-2xl bg-gradient-to-br from-blue-900 to-indigo-900 p-4 ${className}`}
      >
